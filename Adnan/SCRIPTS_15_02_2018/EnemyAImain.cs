...
public class EnemyAImain : MonoBehaviour {
    ...
    private float normalForwardSpeed = 10;
    private float TurboForwardSpeed = 20;
    ...
    private void PassedThePlayer()
    {
       ...
       if (dummycurTimer < 0)
        ...
        {
            if (transform.position.x <= -WidthOfTheThinnerRoad / 4f  + 0.1f) vP.setSideWaysSpeed(0);
        ...
        if (transform.position.x > WidthOfTheThinnerRoad / 4f + 0.1f) vP.setSideWaysSpeed(-3);
        if (transform.position.x < -WidthOfTheThinnerRoad / 4f - 0.1f) vP.setSideWaysSpeed(3);
